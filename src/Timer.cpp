...
bool Timer::Comparator::operator()(const Timer::ptr& lhs, const Timer::ptr& rhs) const
{
    ...
    else if(lhs->next_ < rhs->next_)    return true;
    else if(lhs->next_ > rhs->next_)    return false;
...
Timer::ptr TimerManager::addTimer(uint64_t ms, std::function<void()> func
                                  ,bool recurring) {
...
Timer::ptr TimerManager::addConditionTimer(uint64_t ms, std::function<void()> func
...
{
    MutexLockGuard lock(mutex_);
    if(timer_list_.empty()) return ~0ull;
    Timer::ptr time = *timer_list_.begin();
    uint64_t now_ms = GetCurrentMS();
    if(time->next_ <= now_ms)    return ~0ull;
