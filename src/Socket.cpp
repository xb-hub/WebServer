...
namespace xb
{
    ...
    static xb::Logger::ptr g_logger = GET_LOGGER("system");

    Socket::ptr Socket::CreateTCP(xb::Address::ptr address)
    {
        Socket::ptr sock(new Socket(address->getFamily(), TCP, 0));
        return sock;
    }

    Socket::ptr Socket::CreateUDP(xb::Address::ptr address)
    {
    ...
    bool Socket::bind(const Address::ptr addr)
    {
        ...
        {
            newSock();
            if (XB_UNLIKELY(!isValid()))
            {
                return false;
            }
        }

        if (XB_UNLIKELY(addr->getFamily() != m_family))
        {
        ...
        //         return false;
        //     } else {
        //         xb::FSUtil::Unlink(uaddr->getPath(), true);
        //     }
        // }
    ...
    {
        return IOManager::GetThis()->cancelEvent(m_sock, xb::FdEventType::READ);
    }

    bool Socket::cancelWrite()
    {
        return IOManager::GetThis()->cancelEvent(m_sock, xb::FdEventType::WRITE);
    }
    ...
    SSLSocket::ptr SSLSocket::CreateTCP(xb::Address::ptr address)
    {
