...
public:
    ...
    template<typename T>
    static typename ConfigVar<T>::ptr LookUp(const std::string& name, const std::string& description, const T& value)
    {
        ConfigVarMap& data = getData();
...
private:
                       ...
                       std::vector<std::pair<std::string, YAML::Node>>& node_list)
    {
        auto iter = std::find_if(node_list.begin(), node_list.end(),
                                   [&name](const std::pair<std::string, YAML::Node>& item){ return item.first == name; });
        if(iter != node_list.end())  iter->second = node;
        else    node_list.push_back(std::make_pair(name, node));
        if(node.IsSequence())
        {
